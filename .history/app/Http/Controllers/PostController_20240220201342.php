...
/* The `catch(\Exception )` block in the code is used to catch any exceptions that may occur during
the execution of the code within the `try` block. If an exception of type `Exception` is thrown
within the `try` block, it will be caught by this `catch` block, allowing you to handle the
exception in a specific way. In this case, if an exception is caught, the code redirects back with
an error message indicating that there is an issue making the post and advises to contact the admin. */
        catch(\Exception $e)
        {
            echo($e);
        }
