...
class PostController extends Controller
{
    ...
    public function update(Request $req, $postid)
    {
        ...
        if (!$postid) 
        {
/* The line `return redirect()->route('post.view')->with('error', 'Post not found');` is redirecting
the user to the 'post.view' route with a flash message indicating that the post was not found. The
flash message can be accessed in the view to display an error message to the user. */
            return redirect()->route('post.view')->with('error', 'Post not found');
        }
